...
func (p *Postgres) GetSongs(ctx context.Context, limit, offset int, group, song string) ([]models.Song, error) {
	...
	if err != nil {
		p.log.Errorf("error getting songs: %s", err.Error())
		return nil, err
	}
	...
	for val.Next() {
		...
		if err != nil {
			p.log.Errorf("error scanning into song struct: %s", err.Error())
			return nil, err
		}
...
func (p *Postgres) GetSongText(ctx context.Context, group, song string) (string, error) {
	...
	if err != nil {
		p.log.Errorf("error getting song text from db: %s", err.Error())
		return "", err
	}
	...
	err = val.Scan(&text)
	if err != nil {
		p.log.Errorf("error scanning into text: %s", err.Error())
		return "", err
	}
...
func (p *Postgres) DeleteSong(ctx context.Context, group, song string) error {
	...
	if err != nil {
		p.log.Errorf("error deleting a song: %s", err.Error())
		return err
	}
...
func (p *Postgres) CreateSong(ctx context.Context, song models.Song) (models.Song, error) {
	...
	if err != nil {
		p.log.Errorf("error creating a song: %s", err.Error())
	...
	if !val.Next() {
		p.log.Warnf("song %v already exists", song)
	...
	if err != nil {
		p.log.Errorf("error scanning into models.Song struct: %s", err.Error())
		return models.Song{}, err
	}
...
func (p *Postgres) EditSong(ctx context.Context, editRequest models.EditSongDTO) (models.Song, error) {
	...
	if err != nil {
		p.log.Errorf("error editing a song: %s", err.Error())
		return models.Song{}, err
	}

	if !val.Next() {
		p.log.Warnf("song %s - %s was not found", editRequest.Group, editRequest.Song)
